...
class CodePropUserBasicPrompt(CodePrompt):
    def __init__(self, raw_class, class_name):
        super().__init__(raw_class, class_name)

    def template(self):
        self.add_system_message(SYSTEM_PROMPT)
        self.add_user_message(CODE_PROP_BASIC_PROMPT + CODE_PROP_USER_TASK(self.class_name) + self.classwithprop + CODE_PROP_END_OF_PROMPT)